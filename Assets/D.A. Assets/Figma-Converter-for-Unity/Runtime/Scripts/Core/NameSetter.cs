...
namespace DA_Assets.FCU
{
    ...
    public class NameSetter : FcuBase
    {
        ...
        private ConcurrentDictionary<string, int> _fileNames = new ConcurrentDictionary<string, int>();
        private ConcurrentDictionary<string, int> _fileNameFirstHash = new ConcurrentDictionary<string, int>();
                    ...
                    case FcuNameType.File:
                        {
                            if (fobject.IsSprite())
                            {
                                ...
                                string baseKey = fn.ToLower();

                                int index = 0;

                                if (_fileNameFirstHash.TryGetValue(baseKey, out int storedHash))
                                {
                                    if (storedHash != fobject.Data.Hash)
                                    {
                                        index = _fileNames.AddOrUpdate(baseKey, 1, (_, cur) => cur + 1);
                                    }
                                }
                                else
                                {
                                    _fileNameFirstHash.TryAdd(baseKey, fobject.Data.Hash);
                                    _fileNames.TryAdd(baseKey, 0);
                                }

                                string ext = fobject.Data.ImageFormat.ToLower();
