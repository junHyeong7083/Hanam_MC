...
namespace DA_Assets.FCU
{
    ...
    public class TagSetter : FcuBase
    {
        ...
        public async Task SetTags(FObject page, CancellationToken token)
        {
            ...
            int totalNodes = CountAllNodes(page);

            Debug.Log(FcuLocKey.log_tagging_by_parts.Localize(1));
            await SetTagsAsync(page, TagAlgorithm.Figma, ProgressBarCategory.TaggingFigma, totalNodes, token);

            Debug.Log(FcuLocKey.log_tagging_by_parts.Localize(2));
            await SetTagsAsync(page, TagAlgorithm.Smart, ProgressBarCategory.TaggingSmart, totalNodes, token);

            Debug.Log(FcuLocKey.log_tagging_by_parts.Localize(3));
            await SetTagsAsync(page, TagAlgorithm.Ignore, ProgressBarCategory.TaggingIgnore, totalNodes, token);
        }
            ...
            CancellationToken importToken)
        {
            ...
            monoBeh.EditorDelegateHolder.StartProgress?.Invoke(monoBeh, category, totalCount, false);
            ...
            try
            {
                ...
                TryProcessNode(root, tagAlgorithm, category, importToken);

                await _tcs.Task;
            }
            finally
            {
                monoBeh.EditorDelegateHolder.CompleteProgress?.Invoke(monoBeh, category);
                logCTS.Cancel();
            }
        }
        ...
        {
            Debug.Log(FcuLocKey.log_tagging.Localize(_processedElements));

            try
            {
                while (!logToken.IsCancellationRequested && !importToken.IsCancellationRequested)
                {
                    logToken.ThrowIfCancellationRequested();
        ...
        private void TryProcessNode(FObject node, TagAlgorithm tagAlgorithm, ProgressBarCategory category, CancellationToken importToken)
        {
            if (TryIncrementConcurrency())
            {
                ThreadPool.QueueUserWorkItem(state =>
                {
                    try
                    {
                        ProcessNode(node, tagAlgorithm, category, importToken);
                    }
                    finally
                    {
        ...
        private void ProcessNode(FObject parent, TagAlgorithm tagAlgorithm, ProgressBarCategory category, CancellationToken importToken)
        {
            importToken.ThrowIfCancellationRequested();
            ...
            int currentProcessed = Interlocked.Increment(ref _processedElements);
            monoBeh.EditorDelegateHolder.UpdateProgress?.Invoke(monoBeh, category, currentProcessed);
            ...
            for (int i = 0; i < parent.Children.Count; i++)
            {
                ...
                {
                    Interlocked.Increment(ref _outstandingTasks);
                    TryProcessNode(child, tagAlgorithm, category, importToken);
                }
            }
        }
        ...
        private bool CanBeInsideSingleImage(FObject fobject, FObject parent)
        {
            ...
            {
                reason = 2;
                FcuLogger.Debug($"Reason: {reason} - ForceContainer is true. | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}", FcuDebugSettingsFlags.LogSetTag);
                return false;
            }
            ...
            {
                reason = 3;
                FcuLogger.Debug($"Reason: {reason} - ForceImage is true. | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}", FcuDebugSettingsFlags.LogSetTag);
                return false;
            }
            ...
            foreach (FcuTag fcuTag in fobject.Data.Tags)
            {
                ...
                {
                    reason = 4;
                    FcuLogger.Debug($"Reason: {reason} - Tag {fcuTag} cannot be inside a single image. | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}", FcuDebugSettingsFlags.LogSetTag);
                    return false;
                }
            }
