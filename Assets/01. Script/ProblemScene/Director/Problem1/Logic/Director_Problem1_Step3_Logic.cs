...
/// <summary>
/// Problem1 / Step3 정렬 로직 베이스
/// - RandomCardSequenceStepBase를 상속하여
///   "생각/사실" 분류 + 로그 저장 + 요약 패널 전환까지 처리.
/// - 현재 필름 카드는 카드 인덱스별로 텍스트/이미지를 함께 갱신.
/// </summary>
public abstract class Director_Problem1_Step3_Logic : RandomCardSequenceStepBase
{
    ...
    protected class SortLogEntry
    {
        public int filmId;
        public string text;
        public string correctType;   // "생각" / "사실"
        public string chosenType;    // "생각" / "사실"
    }
    ...
    /// <summary>index에 해당하는 필름의 이미지(Sprite)</summary>
    protected abstract Sprite GetFilmSprite(int index);

    /// <summary>index에 해당하는 필름의 '생각' 여부</summary>
    ...
    private void SpawnOrUpdateCurrentFilmCard(string text, Sprite sprite)
    {
        ...
        // 프리팹 내부 Text 갱신 (문항 텍스트)
        var uiText = _currentFilmInstance.GetComponentInChildren<Text>(true);
        if (uiText != null)
            uiText.text = text ?? string.Empty;

        // 프리팹 내부 Image 갱신 (문항 이미지)
        // 주의: 프리팹에 Image가 여러 개면 의도치 않은 Image가 잡힐 수 있음.
        // 그 경우 전용 View 스크립트로 참조 분리하는 걸 권장.
    ...
    {
        if (parent == null || BinItemPrefab == null)
            return;
