...
public class Director_Problem2_Step1 : ProblemStepBase
{
    ...
    public void NotifyDragEnd(Director_Problem2_DragItem item, PointerEventData eventData)
    {
        ...
        // 드래그 종료 후에는 기본적으로 흰 선 끄기 (UpdateHighlight에서 켜져 있던 것 포함)
        dropBoxArea.SetOutlineVisible(false);
    ...
    private void OnItemDroppedIntoBox(Director_Problem2_DragItem item)
    {
        ...
        if (dropBoxArea != null)
        {
            // UIDropBoxArea가 들고 있는 RectTransform이 있다면 그것을 사용해도 되고,
            // 지금처럼 Step에서 dropBoxArea의 RectTransform을 직접 참조하는 구조로 유지해도 된다.
            // 여기서는 DragItem 쪽 API 그대로 사용.
