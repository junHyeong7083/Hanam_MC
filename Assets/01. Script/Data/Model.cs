...
public class InventoryItem
{
    ...
    public string ItemId { get; set; }      // "mind_lens" 같은 고유 ID
    public string ItemName { get; set; }    // "마음 렌즈" (표시명/프론트용)
    public int ProblemIndex { get; set; }  // 어느 문제(1..N)에서 획득했나 (필요 없으면 null)
    public ProblemTheme Theme { get; set; } // 어느 테마에서 획득했나 등
...
public class Attempt
{
    ...
    // 어떤 문제에 대한 시도인지
    public string Content { get; set; }         // 텍스트/응답 JSON 등
...
public class ProblemFlowSummary
{
    ...
    // 전체 풀이에 걸린 시간(초). 알 수 없으면 null
    public int? DurationSec { get; set; }

    // 최종적으로 성공했는지 여부 (여기서는 항상 true로 나올 수 있으나 추후 확장)
...
public class UserProgress
{
    ...
    {
        // 여기서는 단순 카운터만 증가시켜도 됨.
        // (필요하면 나중에 테마별 푼 수, 최근 푼 문제 목록 등 더 세분화 할 수도)
